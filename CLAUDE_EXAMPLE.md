...
- **每轮回复结束前必须调 `speak_summary`，无例外**。不管是回答问题、写代码、还是闲聊，都要在回复的最后一步调用。headline 要精炼（≤50 字），说人话，不要念代码或路径。服务器返回 `"throttled"` / `"suppressed_quiet_hours"` 等是正常的，照常调就行。**不要自己判断"这个太琐碎不用播报"——这是硬性规则。**
...
- **End every reply with `speak_summary`, no exceptions.** Headline ≤50 words, plain language, no code/paths. Throttle/quiet-hours responses are normal — keep calling.
